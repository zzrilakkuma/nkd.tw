...
## V1.2 迭代任務（2026-08-25 起）

| # | 任務 | 狀態 |
|---|---|---|
| 1 | Header logo 改為 NKD 圖檔（public/nkd-logo.png） | ✅ 完成 |
| 2 | Footer 文案更新（NKD Wholesale Platform + 法規警語） | ✅ 完成 |
| 3 | 新增 Contact Us 頁面 | ✅ 骨架完成（LINE/Email/營業時間待業主提供） |
| 4 | 商品卡：單一 SKU 也顯示規格膠囊 | ✅ 完成 |
| 5 | 商品圖片檔案上傳（POST /images，DB 儲存 3MB 內，重新部署不遺失） | ✅ 完成 |
| 6 | 結帳發票選項（統編 8 碼驗證 + 公司名稱；後台/我的訂單顯示） | ✅ 完成 |
| 7 | 核對階段改單：品項數量調整（同步保留庫存）+ 折扣（total = 小計 − 折扣 + 運費）| ✅ 完成 |

Migration `0008`：orders.discount / orders.invoice + images 表。測試 48 個全過。
